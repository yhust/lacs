...
public class WriterTest {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if(args.length >=4){
      writer = new LoadAwareFileWriter(args[0], new AlluxioURI(String.format("%s/%s", TEST_PATH, args[1])),
              Integer.parseInt(args[2]), Float.parseFloat(args[3]), fileSystem);
    }else {
      System.out.println("Usage: localFilePath, AlluxioURI, workerId, cacheRatio.");
      return;
    }
    writer.writeFile();
    System.exit(0);
